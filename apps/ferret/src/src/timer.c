...
double BSP_time_start(stimer_t *time_now){
	...
	gettimeofday(&(time_now->lat_start),0);

	time_now->what_time = (double)time_now->lat_start.tv_usec + (double)(time_now->lat_start.tv_sec/0.000001);

	return time_now->what_time;
}
double BSP_time_end(stimer_t *time_now){
	time_now->what_time = 0;
	gettimeofday(&(time_now->lat_end),0);

	time_now->what_time = (double)time_now->lat_end.tv_usec + (double)(time_now->lat_end.tv_sec/0.000001);
