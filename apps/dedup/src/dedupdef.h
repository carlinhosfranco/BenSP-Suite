...
//extern long double Ttime_chunk = 0.00000;
...
static inline void sequence_L2_reset(sequence_t *s) {
  assert(s!=NULL);
  //s->l1num=0;
  s->l2num=0;
}
...
typedef struct _chunk_t {
  ...
  struct{
    chunk_state_t FLAG_EOF;
    int idx_file;
    char *file_name;
    size_t file_size;
    sequence_number_t L1_anchorcount;
    sequence_number_t L2_chcount;
  }BSP;
...
typedef struct _metrics_ {

  double time_lapse;
  double time_spent;
  double current_chunk;
  double current_size_processed;
  double total_size_processed;
  double chunks_processed;
  double throughput_by_chunk;
  double throughput_by_size;
...
typedef struct _latency_{
  ...
  int number_of_real_time_collect;
  double leapse_time;
  double service_time;
  double mean_fr_dd;
  double mean_dd_comp;
  ...
  int current_chunk_dd;
  int current_chunk_not_dd;

}latency;

#endif //ENABLE_TRACING
/* Legenda
ITEM_PER_FETCH = É a quantidade de elementos buscados ou inserdos nas filas. Isso é visto nas funções Compress e Reorder. 
QUEUE_SIZE = Usado somente para a execução com Pthreads. O valor é para definir o tamanho das filas entre cada estágio.
...
//BenSP
//#define MAXBUF (1024*1024*1024)     /* 256 MB for buffers */
//#define QUEUE_SIZE 10240UL*1024
//#define ANCHOR_JUMP (1*1024*1024) //best for all 2*1024*1024

//#define MAX_PER_FETCH 1

#define INPUT_FILES_BUFFER 1

#define IN_BUFFER_FR 1
#define OUT_BUFFER_FR 100

#define IN_BUFFER_DD 100
#define OUT_BUFFER_DD 100

#define IN_BUFFER_COMP 100
#define OUT_BUFFER_COMP 100

#define IN_BUFFER_RR 100
#define OUT_BUFFER_RR 100
#define ANCHOR_DATA_PER_INSERT 1

#define SOURCE_FILES 15
