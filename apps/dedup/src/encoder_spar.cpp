...
void *spar_call(void * targs, int NTHREADS) {
  ...
  while (1) {
   ...
   //No data left over from last iteration and also nothing new read in, simply clean up and quit
    ...
    std::vector<chunk_t*> FR_data;
    //partition input block into fine-granular chunks
    [[spar::ToStream,spar::Input(FR_data,rabintab,rabinwintab,NTHREADS)]]do { // Equivalent to Fragment Refine Stage
      ...
      if(offset == 0) {
        ...
        FR_data.push_back(chunk);

        offset = rabinseg(temp->uncompressed_data.ptr, temp->uncompressed_data.n, rf_win_dataprocess, rabintab, rabinwintab);
        //Process the elements if the buffer is full or the end of the loop has been reached.
        if (FR_data.size()==20 || (offset >= temp->uncompressed_data.n && !FR_data.empty())){ 
          [[spar::Stage,spar::Input(FR_data),spar::Output(FR_data),spar::Replicate(NTHREADS)]]
          for(int index = 0; index < FR_data.size(); index++){       
            //Deduplicate 
            int isDuplicate;
            isDuplicate = sub_Deduplicate(FR_data[index]);
            
            #ifdef ENABLE_STATISTICS
            if(isDuplicate) {
              stats.nDuplicates++;
            } else {
              stats.total_dedup += FR_data[index]->uncompressed_data.n;
            }
            #endif //ENABLE_STATISTICS

            if(!isDuplicate) {
              sub_Compress(FR_data[index]); 
              #ifdef ENABLE_STATISTICS
              stats.total_compressed += FR_data[index]->compressed_data.n;
              #endif //ENABLE_STATISTICS
            }
          }

          [[spar::Stage,spar::Input(FR_data)]]
          for(int index = 0; index < FR_data.size(); index++){ 
            write_chunk_to_file(fd_out, FR_data[index]);
            if(FR_data[index]->header.isDuplicate)
              free(FR_data[index]);
          }
        }
        if(FR_data.size()==20) FR_data.clear();
